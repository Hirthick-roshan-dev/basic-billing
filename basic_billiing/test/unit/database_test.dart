...
void main() {
  ...
  group('Database & Repositories Integration Tests', () {
    ...
    test(
      'Settings repository creates and persists default and updated values',
      () async {
    ...
    test(
      'Migration from v2 to v3 safely adds vehicle_number and job_card_no columns',
      () async {
        const dbName = 'test_v2_migration.db';
    ...
    test(
      'Migration from v4 to v5 safely adds payment_type column with default Cash',
      () async {
        const dbName = 'test_v4_migration.db';
        await databaseFactory.deleteDatabase(dbName);
    ...
    test(
      'Offer products CRUD operations work properly across 3 offer groups',
      () async {
        ...
        await offerRepo.addOfferProduct(
          p2,
          offerGroup: 2,
        ); // p2 in both Offer 1 and Offer 2
        ...
        expect(
          await offerRepo.isOfferProduct('Oil Filter', offerGroup: 1),
          isTrue,
        );
        expect(
          await offerRepo.isOfferProduct('Brake Pads', offerGroup: 1),
          isTrue,
        );
        expect(
          await offerRepo.isOfferProduct('Engine Oil', offerGroup: 1),
          isFalse,
        );
        ...
        final groupsForBrakePads = await offerRepo.getOfferGroupsForProduct(
          'Brake Pads',
        );
        ...
        expect(
          await offerRepo.isOfferProduct('Brake Pads', offerGroup: 2),
          isTrue,
        );
    ...
    test(
      'Offer products are listed in insertion order (first added first, last added last)',
      () async {
        await offerRepo.clearOfferProducts(offerGroup: 4);

        final pZ = ProductModel(
          id: 101,
          name: 'Zebra Product',
          price: 100.0,
          createdAt: DateTime.now(),
        );
        final pA = ProductModel(
          id: 102,
          name: 'Alpha Product',
          price: 200.0,
          createdAt: DateTime.now(),
    ...
    test(
      'Migration from v5 to v6 safely creates offer_products table',
      () async {
    ...
    test(
      'Migration from v6 to v7 safely adds offer_group column to offer_products table',
      () async {
        const dbName = 'test_v6_migration.db';
        await databaseFactory.deleteDatabase(dbName);
    ...
    test(
      'Migration from v7 to v8 safely creates offer_groups table with seeded defaults',
      () async {
        const dbName = 'test_v7_migration.db';
        await databaseFactory.deleteDatabase(dbName);
    ...
    test(
      'Migration from v8 to v9 safely adds total_purchase_amount to bills and purchase_price to bill_items',
      () async {
        const dbName = 'test_v8_migration.db';
        await databaseFactory.deleteDatabase(dbName);
          ...
          options: OpenDatabaseOptions(
            ...
            onCreate: (db, version) async {
              ...
              CREATE TABLE bills (
                ...
                is_total_edited INTEGER NOT NULL DEFAULT 0,
                created_at TEXT NOT NULL,
        ...
        // Insert legacy data into v8 database
        await v8Db.insert('bills', {
        ...
        await v8Db.insert('bill_items', {
          'bill_id': 1,
          'product_name': 'Legacy Engine Oil',
          'unit_price': 500.0,
          'quantity': 1,
          'total_price': 500.0,
        });
        ...
        // Run migration to v9
        await DatabaseMigrations.onUpgrade(v8Db, 8, 9);

        // Verify legacy bill read retains data and has total_purchase_amount default to 0.0
        final billsResult = await v8Db.query('bills');
        expect(billsResult.first['invoice_number'], 'INV-LEGACY-001');
        expect(billsResult.first['total_purchase_amount'], 0.0);

        final itemsResult = await v8Db.query('bill_items');
        expect(itemsResult.first['product_name'], 'Legacy Engine Oil');
        expect(itemsResult.first['purchase_price'], 0.0);

        // Verify inserting new records with v9 fields works
        final billingRepo = BillingRepository(db: v8Db);
        final legacyBill = await billingRepo.getBillWithItems(1);
        expect(legacyBill, isNotNull);
        expect(legacyBill!.totalPurchaseAmount, 0.0);
        expect(legacyBill.items.first.purchasePrice, 0.0);
    ...
    test(
      'Migration from v9 to v10 safely adds purchase_shop_name and purchase_payment_type',
      () async {
        const dbName = 'test_v9_migration.db';
        await databaseFactory.deleteDatabase(dbName);
        final v9Db = await databaseFactory.openDatabase(
          dbName,
          options: OpenDatabaseOptions(
            version: 9,
            onCreate: (db, version) async {
        ...
        await DatabaseMigrations.onUpgrade(v9Db, 9, 10);

        final billsResult = await v9Db.query('bills');
        expect(billsResult.first['invoice_number'], 'INV-LEGACY-002');
        expect(billsResult.first.containsKey('purchase_shop_name'), isTrue);
        expect(billsResult.first.containsKey('purchase_payment_type'), isTrue);
        expect(billsResult.first['purchase_payment_type'], 'Cash');
    ...
    test(
      'isJobCardNumberExists detects duplicates and respects excludeBillId',
      () async {
        final billingRepo = BillingRepository(db: db);

        // Initially no job card exists
        expect(await billingRepo.isJobCardNumberExists('JC-9999'), isFalse);

        // Create a bill with JC-9999
        final created = await billingRepo.createBill(
          BillModel(
            invoiceNumber: 'INV-JC-001',
            createdAt: DateTime.now(),
            jobCardNumber: 'JC-9999',
            subtotal: 500.0,
            totalAmount: 500.0,
            purchaseShopName: 'Sri Motors',
        ...
        // Now JC-9999 exists (case-insensitive)
        expect(await billingRepo.isJobCardNumberExists('jc-9999'), isTrue);
        expect(await billingRepo.isJobCardNumberExists('JC-9999'), isTrue);
        expect(await billingRepo.isJobCardNumberExists(' JC-9999 '), isTrue);

        // When editing the same bill, it should be excluded
        expect(
          await billingRepo.isJobCardNumberExists('JC-9999', excludeBillId: created.id),
          isFalse,
        );

        // When editing a different bill id, it should detect the conflict
        expect(
          await billingRepo.isJobCardNumberExists('JC-9999', excludeBillId: 99999),
          isTrue,
        );
      },
    );
